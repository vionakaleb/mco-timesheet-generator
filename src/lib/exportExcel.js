...
import { LOGO_BASE64, LOGO_EXTENSION, LOGO_WIDTH, LOGO_HEIGHT } from '../assets/logo';
...
export async function exportTimesheet({ role, name, month, year, departmentHead, counterSign, calendar, activities, hours }) {
  ...
  paintMerged(ws, headerRow, 1, weekdayRow, 1, 'Project Name\nActivity Description', FONT_BOLD, CENTER);
  paintMerged(ws, headerRow, sumCol, weekdayRow, sumCol, 'Sum (hours)', FONT_BOLD, CENTER);
  paintMerged(ws, headerRow, counterCol, weekdayRow, counterCol, 'Counter Sign Name', FONT_BOLD, CENTER);
  paintMerged(ws, headerRow, signCol, weekdayRow, signCol, 'Signature', FONT_BOLD, CENTER);
  ...
  activities.forEach((desc, i) => {
    ...
    paint(ws, firstActivity + i, 1, desc, FONT, LEFT);
  });

  calendar.forEach((d, i) => {
    const value = hours[d.day];
    const cellValue = isNumeric(value) ? Number(value) : value;
    paintMerged(ws, firstActivity, dayStart + i, lastActivity, dayStart + i, cellValue, FONT, CENTER);
  });
  paintMerged(ws, firstActivity, sumCol, lastActivity, sumCol, sumHours(hours), FONT_BOLD, CENTER);
  paintMerged(ws, firstActivity, counterCol, lastActivity, counterCol, counterSign, FONT, CENTER);
  paintMerged(ws, firstActivity, signCol, lastActivity, signCol, '', FONT, CENTER);

  paintMerged(ws, totalRow, 0, totalRow, 1, 'TOTAL', FONT_BOLD, CENTER);
  calendar.forEach((d, i) => paint(ws, totalRow, dayStart + i, dayTotal(hours[d.day]), FONT_BOLD, CENTER));
  ...
  for (let i = 0; i < dayCount; i += 1) ws.getColumn(dayStart + i + 1).width = 5;
  ...
  for (let r = firstActivity; r <= lastActivity; r += 1) ws.getRow(r + 1).height = 30;

  const logoWidth = Math.round((LOGO_PIXEL_HEIGHT * LOGO_WIDTH) / LOGO_HEIGHT);
  const imageId = wb.addImage({ base64: LOGO_BASE64, extension: LOGO_EXTENSION });
  ws.addImage(imageId, {
    tl: { col: 0.2, row: 0.3 },
